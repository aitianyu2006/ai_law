...
from .data_util_test import token_string_as_list, imprisonment_mean, imprisonment_std, UNK_ID, load_word_vocab, \
    load_label_dict_accu, load_label_dict_article, pad_truncate_list
...
class Predictor(object):
    def __init__(self):
        ...
        self.model = HierarchicalAttention(accusation_num_classes, article_num_classes, deathpenalty_num_classes,
                                           lifeimprisonment_num_classes, FLAGS.learning_rate, self.batch_size,
                                           FLAGS.decay_steps, FLAGS.decay_rate, FLAGS.sentence_len, FLAGS.num_sentences,
                                           vocab_size, FLAGS.embed_size, FLAGS.hidden_size
                                           , num_filters=FLAGS.num_filters, model=FLAGS.model,
    ...
    def predict_with_model_batch(self, contents):
        ...
        for i, fact in enumerate(contents_padded):
            ...
            x = pad_truncate_list(x, self.FLAGS.sentence_len, value=0.,
                                  truncating='pre')  # padding to max length.remove sequence that longer than max length from beginning.
            input_X.append(x)
        ...
        logits_accusations, logits_articles, logits_deathpenaltys, logits_lifeimprisonments, logits_imprisonments = self.sess.run(
            [model.logits_accusation, model.logits_article, model.logits_deathpenalty, model.logits_lifeimprisonment,
             model.logits_imprisonment], feed_dict)
        ...
        for i in range(length_contents):
            ...
            accusations_predicted = [j + 1 for j in range(len(logits_accusation)) if
                                     logits_accusation[j] >= 0.5]  # TODO ADD ONE e.g.[2,12,13,10]
            ...
            articles_predicted = [j + 1 for j in range(len(logits_article)) if
                                  logits_article[j] >= 0.5]  ##TODO ADD ONE e.g.[2,12,13,10]
...
if __name__ == '__main__':
    ...
    predict = Predictor()
    content1 = u"武强县人民检察院指控：2010年12月21日10时许，被告人王某趁武强县金音建筑工地工友张某不在宿舍之际，盗窃张某枕头下现金3500元。"
